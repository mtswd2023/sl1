...
case object BstTests extends Assignment {
  ...
  override def props: List[(String, L1PropOrTest)] = {
    val arbitraryBst =
      Gen.listOf(Gen.choose(0.0, 100.0).flatMap(value => Gen.alphaStr.map(label => (value, label)))).map(Bst.create)

    List(
      "contains returns true for inserted elements" -> L1Prop(
        Prop.forAll(arbitraryBst, Gen.choose(0.0, 100.0)) {
          (
            bst: Bst,
            value: Double,
          ) =>
      ...
      "labelOf returns None for non-inserted elements" -> L1Prop(
        Prop.forAll(arbitraryBst, Gen.choose(0.0, 100.0)) {
          (
            ...
            bst.labelOf(value) == Option.empty
        },
      ),
