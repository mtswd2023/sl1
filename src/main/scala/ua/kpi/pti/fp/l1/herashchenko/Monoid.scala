...
object Monoid {
    ...
    def apply[A,F[_]](implicit monoid: Monoid[F[A]]): Monoid[F[A]] = monoid
    
    implicit def genericMonoid[A,F[_]](implicit empt: Empty[F], comb: Combine[F]) = new Monoid[F[A]]{
        override def combine(x: F[A], y: F[A]): F[A] = comb.combine(x,y)
...
object Empty{
    def apply[F[_]](implicit instance: Empty[F]): Empty[F] = instance
...
object Combine{
        def apply[F[_]](implicit instance: Combine[F]): Combine[F] = instance
